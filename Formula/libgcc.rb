class Libgcc < Formula
  ...
  def install
    ...
    ENV["CFLAGS"]   = "--sysroot=#{sysroot} -mstrict-align -fPIC"
    ENV["CXXFLAGS"] = "--sysroot=#{sysroot} -mstrict-align -fPIC"
    ENV["LDFLAGS"]  = "--sysroot=#{sysroot} -Wl,--code-sign"
    # 构建工具链也用 strict-align：clang 可能对 GCC 数据结构
    # 生成 LDST64 重定位，但实际只有 4 字节对齐 → lld 拒绝
    ENV["CFLAGS_FOR_BUILD"] = "-mstrict-align -fPIC"
    ENV["CXXFLAGS_FOR_BUILD"] = "-mstrict-align -fPIC"
    ...
    mkdir "build" do
      ...
      make_args = [
        "CFLAGS=--sysroot=#{sysroot} -mstrict-align -fPIC",
        "CXXFLAGS=--sysroot=#{sysroot} -mstrict-align -fPIC",
        "CFLAGS_FOR_BUILD=-mstrict-align -fPIC",
        "CXXFLAGS_FOR_BUILD=-mstrict-align -fPIC",
